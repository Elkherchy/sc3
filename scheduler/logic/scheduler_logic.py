...
def generate_schedule():
    ...
    for g in groupes:
        for m in matieres:
            solver.Add(sum(X[g.groupe_id, m.matiere_id, d, t] for d in days for t in slots) == 1)
        ...
        for g in groupes:
            for m in matieres:
                if MatiereTeacher.objects.filter(enseignant=disp.enseignant, matiere=m).exists():
                    solver.Add(X[g.groupe_id, m.matiere_id, disp.jour_semaine, disp.creneau_horaire] <= 1)
